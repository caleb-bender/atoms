...
# Atoms Data Model Classes (3.x.x)
## Introduction
In Atoms, data model classes (also commonly known as Data Transfer Objects or DTOs) are the foundational building blocks that make the Atoms ORM so powerful. For instance, one of the core interfaces you will utilize is the `IAtomicRepository<TModel>`. As of version 3.x.x, the interface definition is below:
```csharp
...
As a note, the current property types that are compatible with the `UniqueIdAttribute` are all primitives, all enum types, and `string, decimal, TimeOnly, DateTime, or Guid`. 
...
## Anonymous Data Entity Models

If you want to use a class that is not annotated with the `DbEntityNameAttribute`, you can create a class without the attribute (still needs at least one property with the `UniqueIdAttribute`) and then use `CreateRepository` method by supplying the database entity name as a string. For example:
...
This is useful when you have a legacy system that contains multiple tables with the same schema, but different names. You can use the same data model class for each table by creating separate repository instances with the appropriate database entity name. In this way, you can avoid creating multiple data model classes that are identical in structure.


## FakeAtomicRepository for Unit Testing

Writing fast unit tests is critical for quick feedback during refactoring and development. However, when your code depends on a database, tests can become slow and brittle. To solve this problem, Atoms provides the `FakeAtomicRepository<TModel>` class, which implements the `IAtomicRepository<TModel>` interface and allows you to perform in-memory CRUD operations without needing a real database connection. Here is how you can use it in an xUnit test:
...
{
    readonly FakeAtomicRepository<User> _userRepository;
    readonly List<User> _initialUsers = new()
    {
    ...
    {
        _userRepository = new FakeAtomicRepository<User>(_initialUsers);
    }
    ...
    public async Task GetOneAsync_ReturnsCorrectUser(int userId, string expectedName)
    {
        var userToFind = new User { Id = userId };
        var option = await _userRepository.GetOneAsync(userToFind);
        if (option is not AtomicOption<User>.Exists existingUserOption)
        { Assert.True(false, $"User with Id {userId} should exist."); return; }
        Assert.Equal(expectedName, existingUserOption.Value.Name);
    }
}
```

Creating a `FakeAtomicRepository<TModel>` instance is as simple as passing in a required initial list of models. The repository will use this list as its in-memory data store. You can then call the `CreateManyAsync`, `GetOneAsync`, `UpdateManyAsync`, and `DeleteManyAsync` methods just like you would with a real database repository, as well as the extension methods. For checking state changes you can use LINQ queries on the initial list of models.

```csharp
[Fact]
public async Task CreateManyAsync_AddsNewUsers()
{
    ...
    Assert.Equal(5, _initialUsers.Count);
    Assert.Contains(_initialUsers, u => u.Id == 4 && u.Name == "David");
    Assert.Contains(_initialUsers, u => u.Id == 5 && u.Name == "Eve");
}
```
